				...
				if((null != zwwldevicesList) && (!zwwldevicesList.isEmpty())){
					...
					if(null == blueToothBraceletData){
						blueToothBraceletData = new BlueToothBraceletData(deveui, step, caloric, Float.parseFloat(gatewayX),Float.parseFloat(gatewayY),
								                                           Short.parseShort(heartBeat), Short.parseShort(systolicPressure), Short.parseShort(diastolicPressure), distance,new Date(),battery+"%",Short.parseShort(rssi),new Date(),new Date());
						...
						if(!blueToothBraceletData.getBattery().equals(battery)){
							...
							blueToothBraceletData.setBatteryTime(new Date());
						}else{
							//have a problem
							long batteryNumber = new Date().getTime() - blueToothBraceletData.getBatteryTime().getTime();
							if((batteryNumber % zwwldevices.getEnergycyclevalue()) == 0){
								ZwwlSender.sendBatterythrData(deveui, battery,zwwldevices.getTenantId());
							}
						}
						
						if(!blueToothBraceletData.getDistance().equals(distance)){
							ZwwlSender.sendGWPosAndDistanceData(deveui, gatewayX, gatewayY, distance, tenantId);
							blueToothBraceletData.setDistance(distance);
							blueToothBraceletData.setDistanceTime(new Date());
						}else{
							long distanceNumber = new Date().getTime() - blueToothBraceletData.getDistanceTime().getTime();
							if((distanceNumber % zwwldevices.getEnergycyclevalue()) == 0 ){
								ZwwlSender.sendGWPosAndDistanceData(deveui, gatewayX, gatewayY, distance, tenantId);
							}
						}
